...
/// pushed onto `snapshots` and wrapped up in an `Rc`. At that point we clone
...
pub(crate) struct SnapshotList<T> {
    ...
    // the `Rc` wrapper around sub-lists, which makes cloning time for this
...
impl<T> SnapshotList<T> {
    ...
    pub(crate) fn commit(&mut self) -> SnapshotList<T> {
        ...
        // `Rc`-wrapped vector in the snapshots list. Note the `shrink_to_fit`
        ...
        if len > 0 {
            ...
            self.snapshots.push(Rc::new(Snapshot {
